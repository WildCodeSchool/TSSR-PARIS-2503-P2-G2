...
while [ "$choix" != "6" ]; do
        case "$choix" in
                ...
                source $(pwd)/Gestion_Utilisateur.sh
                ;;
                2) 
                #Gestion des groupes
                source $(pwd)/Gestion_Groupe.sh
                ;;
                3) 
                #Informations des utilisateurs
                source $(pwd)/Informations_Utilisateur.sh
                ;;
                4) 
                #Gestion des répertoires
                source $(pwd)/Gestion_repertoire.sh
                ;;
                5) 
                #Gestion droits et permissions
                source $(pwd)/Gestion_DroitsPermission.sh
                ;;
                6) 
