...
sudo echo "$DATE-$HEURE-$USER-$1 : demande IP de la machine distante : $ip" >> $LOGFILE

read -p "Nom d'utilisateur SSH : " ssh_user
sudo echo "$DATE-$HEURE-$USER-$1 : demande nom utilisateur de la machine distante : $ssh_user" >> $LOGFILE

# Fichier/dossier et utilisateur cible
read -p "Veuillez indiquer le chemin du fichier/dossier : " chemin 
sudo echo "$DATE-$HEURE-$USER-$1 : demande path du fichier/dossier : $chemin" >> $LOGFILE

read -p "Veuillez indiquer l'utilisateur concerné : " user 
sudo echo "$DATE-$HEURE-$USER-$1 : demande nom utilisateur concerne : $user" >> $LOGFILE
...
sudo echo "$DATE-$HEURE-$USER-$1 : test existence fichier dans le systeme" >> $LOGFILE
...
if [ $? -ne 0 ]; then
    ...
    sudo echo "$DATE-$HEURE-$USER-$1 : Erreur, le fichier/dossier ou l'utilisateur n'existe pas sur la machine" >> $LOGFILE
    exit 1
else
    echo "Vérification reussie."
    sudo echo "$DATE-$HEURE-$USER-$1 : Verification existence fichier/dossier reussie" >> $LOGFILE
...
sudo echo "$DATE-$HEURE-$USER-$1 : réponse de l'utilisateur $choix" >> $LOGFILE
...
case "$choix" in
    add)
                ...
                sudo echo "$DATE-$HEURE-$USER-$1 : choix droit $choix_add" >> $LOGFILE
            ...
            case "choix" in
                    ...
                    sudo echo "$DATE-$HEURE-$USER-$1 : ajout droit lecture réussis" >> $LOGFILE
                    ;;
                    +w)
                    ssh ${ssh_user}@${ip} "echo '$sudo_pass' | sudo -S chmod u+w '$chemin' && echo 'Écriture ajouté avec succès.'"
                    sudo echo "$DATE-$HEURE-$USER-$1 : ajout droit ecriture réussis" >> $LOGFILE
                    ;;
                    +x)
                    ssh ${ssh_user}@${ip} "echo '$sudo_pass' | sudo -S chmod u+x '$chemin' && echo 'Exécution ajouté avec succès.'"
                    sudo echo "$DATE-$HEURE-$USER-$1 : ajout droit execution réussis" >> $LOGFILE
                    ;;
                    retour)
                    Echo "Retour menu précédent."
                    sudo echo "$DATE-$HEURE-$USER-$1 : retour menu precedent" >> $LOGFILE
                    sleep 3
                    ;;
            esac
            done
    ...
    del)
                ...
                sudo echo "$DATE-$HEURE-$USER-$1 : choix utilisateur $choix_del" >> $LOGFILE
            ...
            case "$choix" in
                    ...
                    sudo echo "$DATE-$HEURE-$USER-$1 : suppression droit lecture réussis" >> $LOGFILE
                    ;;
                    -w)
                    ssh ${ssh_user}@${ip} "echo '$sudo_pass' | sudo -S chmod u-w '$chemin' && echo 'Écriture supprimé avec succès.'"
                    sudo echo "$DATE-$HEURE-$USER-$1 : suppression droit ecriture réussis" >> $LOGFILE
                    ;;
                    -x)
                    ssh ${ssh_user}@${ip} "echo '$sudo_pass' | sudo -S chmod u-x '$chemin' && echo 'Exécution supprimé avec succès.'"
                    sudo echo "$DATE-$HEURE-$USER-$1 : suppression droit execution réussis" >> $LOGFILE
                    ;;
                    retour)
                    echo "Retour menu précédent."
                    sudo echo "$DATE-$HEURE-$USER-$1 : retour menu precedent" >> $LOGFILE
    ...
    retour)
            echo "Retour menu précédent."
            sudo echo "$DATE-$HEURE-$USER-$1 : retour menu principale" >> $LOGFILE
            sleep 3
    ;;
